...
static void esp_spp_cb(esp_spp_cb_event_t event, esp_spp_cb_param_t *param)
{
	switch (event)
	{
		...
		case ESP_SPP_CLOSE_EVT:    // Client connection closed
			...
			{
				log_i("ESP_SPP_CLOSE_EVT status:%d handle:%d close_by_remote:%d attempt %u", param->close.status, param->close.handle, param->close.async, secondConnectionAttempt);
...
void BluetoothSerial::onData(BluetoothSerialDataCb cb) { custom_data_callback = cb; }
	...
	switch (event)
	{
		case ESP_BT_GAP_DISC_RES_EVT:
		{
			...
			for (int i = 0; i < param->disc_res.num_prop; i++)
			{
				...
				if (_isRemoteAddressSet) break;
			}
			if (peer_bdname_len) advertisedDevice.setName(peer_bdname);
			esp_bd_addr_t addr;
			memcpy(addr, param->disc_res.bda, ESP_BD_ADDR_LEN);
			advertisedDevice.setAddress(BTAddress(addr));
			if (scanResults.add(advertisedDevice) && advertisedDeviceCb) advertisedDeviceCb(&advertisedDevice);
		}
		break;
		...
		case ESP_BT_GAP_CFM_REQ_EVT:
			...
			{
                ESP_LOGE("","THIS SHOUD NOT HAVE COME HERE! - SSP DISABLED");
...
static bool _stop_bt()
{
	if (btStarted())
	{
		if (_spp_client) esp_spp_disconnect(_spp_client);
		esp_spp_deinit();
...
bool BluetoothSerial::hasClient(void) { return _spp_client > 0; }
...
size_t BluetoothSerial::write(uint8_t c) { return write(&c, 1); }
...
void BluetoothSerial::end() { _stop_bt(); }

void BluetoothSerial::onConfirmRequest(ConfirmRequestCb cb) { confirm_request_callback = cb; }

void BluetoothSerial::onAuthComplete(AuthCompleteCb cb) { auth_complete_callback = cb; }
...
void BluetoothSerial::enableSSP() { _enableSSP = true; }
/*
...
bool BluetoothSerial::connected(int timeout) { return waitForConnect(timeout); }

/**
 * true if a connection terminated or a connection attempt failed
 */
bool BluetoothSerial::isClosed() { return xEventGroupGetBits(_spp_event_group) & SPP_CLOSED; }
...
{
	scanResults.clear();
	if (timeoutMs < MIN_INQ_TIME || timeoutMs > MAX_INQ_TIME || strlen(_remote_name) || _isRemoteAddressSet) return nullptr;
...
{
	scanResults.clear();
	if (strlen(_remote_name) || _isRemoteAddressSet) return false;
	int timeout = timeoutMs / INQ_TIME;
	disconnect();
...
void BluetoothSerial::discoverClear() { scanResults.clear(); }
 ...
 */
BTScanResults *BluetoothSerial::getScanResults() { return &scanResults; }

BluetoothSerial::operator bool() const { return true; }
 ...
 */
String BluetoothSerial::getBtAddressString() { return getBtAddressObject().toString(true); }
#endif
